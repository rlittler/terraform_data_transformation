...
// Datalake Private Endpoint 
//----------------------------------------------------------------------------------------------------------------------
resource "azurerm_subnet" "private_link_endpoint_storage" {
  ...
  address_prefix                                 = "${var.net_endpoint_sub_cidr_storage}"
...
resource "azurerm_private_endpoint" "plink_storage" {
  ...
  subnet_id           = "${azurerm_subnet.private_link_endpoint_storage.id}"
  ...
  private_service_connection {
    ...
    private_connection_resource_id = "${azurerm_storage_account.dataLakeGen2Storage.id}"
...
resource "azurerm_private_dns_zone" "plink_dns_private_zone_storage" {
...
data "azurerm_private_endpoint_connection" "plinkconnection_storage" {

  name                = "${azurerm_private_endpoint.plink_storage.name}"

  resource_group_name = "${azurerm_private_endpoint.plink.resource_group_name}"

}

resource "azurerm_private_dns_a_record" "private_endpoint_a_record_storage" {

  name                = "storage"
  //"${azurerm_storage_account.dataLakeGen2Storage.name}"

  zone_name           = "${azurerm_private_dns_zone.plink_dns_private_zone_storage.name}"

  resource_group_name = "${azurerm_private_endpoint.plink.resource_group_name}"

  ttl                 = 300

  records             = ["${data.azurerm_private_endpoint_connection.plinkconnection_storage.private_service_connection.0.private_ip_address}"]

}

resource "azurerm_private_dns_zone_virtual_network_link" "zone_to_vnet_link_storage" {

  name                  = "storagelink"

  resource_group_name   = "${azurerm_private_endpoint.plink.resource_group_name}"

  private_dns_zone_name = "${azurerm_private_dns_zone.plink_dns_private_zone_storage.name}"

  virtual_network_id    = "${module.network.vnet_id}"

}

output "private_link_endpoint_ip_storage" {

  value = "${data.azurerm_private_endpoint_connection.plinkconnection_storage.private_service_connection.0.private_ip_address}"

}

# output "private_link_endpoint_storage_ip" {
